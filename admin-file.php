...
function fbcomment_init(){
	...
	$new_fboptn = array_merge($new_fboptn,$post_types);
...
// print_r($fboptn);
	...
	$fb_adminUrl = get_admin_url()."options-general.php?page=". __( "fbcomment", "facebook-comment-by-vivacity" );
		...
		<p>'. __( "Please Enter Your Facebook App ID. Required for Fb comments.", "facebook-comment-by-vivacity" ).' 
		<a href="'.$fb_adminUrl.'">'. __( "Click here", "facebook-comment-by-vivacity" ).'</a>
		</p>
      </div>';
	}  
}
...
function fbcomment_option() {	?>
	...
	<div class="wrap">
		<div class="top">
			<h3> <?php _e( "FB Comments Plugin", "facebook-comment-by-vivacity" ) ?> <small><?php _e("by","facebook-comment-by-vivacity") ?> 
				<a href="http://www.vivacityinfotech.net" target="_blank">Vivacity InfoTech Pvt. Ltd.</a>
			</h3>
		</div> <!-- ------End of top-----------  -->
		<div class="inner_wrap">
			<div class="left">
				<form method="post" action="options.php" id="options">
					<?php
					settings_fields('fbcomment_option'); 
					$fboptn = get_option('fbcomment'); 
					...
					if ( (!isset($fboptn['pagesid'])) || (!is_numeric($fboptn['pagesid'])) ) {$fboptn['pagesid'] = 00;}
					if (!isset($fboptn['archive'])) {$fboptn['archive'] = "";}
					if (!isset($fboptn['posttypes'])) {$fboptn['posttypes'] = "";} 
					if (!isset($fboptn['admin_email'])) {$fboptn['admin_email'] = "";}
					...
					?>
						<div class="error">
							<h3 class="setup"><?php _e("Set Up Your Facebook App ID","facebook-comment-by-vivacity") ?></h3>
							...
							<table class="form-table admintbl">
								<tr>
									<th>
      								...
      								<?php _e("Create an App","facebook-comment-by-vivacity") ?>
      								</a>
      								</strong><br>
									...
									<td>
										<small><?php _e("Enter App Id into below textbox.","facebook-comment-by-vivacity") ?></small><br>
							...
							<!-- -----End Set Up Your Facebook App ID -->
						</div>
			<?php }
					else { ?>
						<h3 class="title" id="fbappsettings"><?php _e("Facebook App Setting","facebook-comment-by-vivacity") ?></h3>
			...
			<?php	} ?>	
					...
					<div  id="mainsettingstbl" class="togglediv">
						<table class="form-table admintbl">
							...
							if ($fboptn['appID']!="") { ?>
								<tr>
									<th><label for="appID"><?php _e("Facebook App ID","facebook-comment-by-vivacity") ?></label></th>
									<td><input id="appID" type="text" name="fbcomment[appID]" value="<?php echo $fboptn['appID']; ?>" /></td>
								</tr>
<?php } ?> 
								<tr>
									<th><label for="admin_email"><?php _e("Notification Email Id","facebook-comment-by-vivacity") ?></label></th>
									<td><input id="admin_email" type="text" name="fbcomment[admin_email]" value="<?php echo $fboptn['admin_email']; ?>" /></td>
								</tr>
								<tr>
									<th><label for="fbml"><?php _e("Enable XFBML","facebook-comment-by-vivacity") ?></label></th>
									<td><input id="fbml" name="fbcomment[fbml]" type="checkbox" value="on" <?php checked('on', $fboptn['fbml']); ?> /> 
									<small><?php _e("only disable this if you already have XFBML enabled elsewhere","facebook-comment-by-vivacity") ?></small>
									</td>
								</tr>
								<tr>
									<th><label for="fbns"><?php _e("Use Facebook NameServer","facebook-comment-by-vivacity") ?></label></th>
									<td><input id="fbns" name="fbcomment[fbns]" type="checkbox" value="on" <?php checked('on', $fboptn['fbml']); ?> /> 
									<small><?php _e("only enable this if Facebook Comments do not appear","facebook-comment-by-vivacity") ?></small>
									</td>
								</tr>
								<tr>
									<th><label for="opengraph"><?php _e("Use Open Graph NameServer","facebook-comment-by-vivacity") ?></label></th>
									<td>
									<input id="opengraph" name="fbcomment[opengraph]" type="checkbox" value="on" <?php checked('on', $fboptn['opengraph']); ?> /> <small><?php _e("only enable this if Facebook comments are not appearing, not all information is being passed to Facebook or if you have not enabled Open Graph elsewhere within WordPress","facebook-comment-by-vivacity") ?></small>
									</td>
								</tr>
								<tr>
									<th><label for="html5"><?php _e("Use HTML5","facebook-comment-by-vivacity") ?></label></th>
									<td><input id="html5" name="fbcomment[html5]" type="checkbox" value="on" <?php checked('on', $fboptn['html5']); ?> /></td>
								</tr>
								<tr>
									<th><label for="pluginsite"><?php _e("Show plugin site url","facebook-comment-by-vivacity") ?>:</label></th>
									<td>
									<input id="credit" name="fbcomment[pluginsite]" type="checkbox" value="on" <?php checked('on', $fboptn['pluginsite']); ?> /> <small><?php _e("only enable this if you want to show plugin site","facebook-comment-by-vivacity") ?>.</small>
		</td>
								</tr>
						</table> 
					</div>
					...
					<h3 id="displaysettings" class="title"><?php _e("Display Settings","facebook-comment-by-vivacity") ?></h3>
					<!-- ---Display Settings -->
					<div id="displaysettingstbl" class="togglediv">
						<table class="form-table admintbl">
							<tr>
								<th><label for="posts"><?php _e( 'Posts', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="posts" name="fbcomment[posts]" type="checkbox" value="on" <?php checked('on', $fboptn['posts']); ?> /></td>
							</tr>
							<tr>
								<th><label for="pages"><?php _e( 'Pages', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="pages" name="fbcomment[pages]" type="checkbox" value="on" <?php checked('on', $fboptn['pages']); ?> /></td>
							</tr>
							<tr>
								<th><label for="homepage"><?php _e( 'Home', 'facebook-comment-by-vivacity' ); ?></label></th>
...
<!-- ------End Post type Settings--------- -->		
							...
							<tr>
								<th><label for="scheme"><?php _e( 'Colour Scheme', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td>
									<select name="fbcomment[scheme]">
										<option value="light" <?php if ($fboptn['scheme'] == 'light') { echo 'selected="selected"'; } ?> >
										<?php _e( 'Light', 'facebook-comment-by-vivacity' ); ?></option>
										<option value="dark" <?php if ($fboptn['scheme'] == 'dark') { echo 'selected="selected"'; } ?> >
										<?php _e( 'Dark', 'facebook-comment-by-vivacity' ); ?></option>
									</select>
								</td>
							</tr>
							<tr>
								<th><label for="num"><?php _e( 'Number of Comments', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="num" type="text" name="fbcomment[num]" value="<?php echo $fboptn['num']; ?>" /> 
								<small> - <?php _e( 'Default no of comments is', 'facebook-comment-by-vivacity' ); ?> <strong>6	</strong></small>
								</td>
							</tr>
							<tr>
								<th><label for="width"><?php _e( 'Width', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="width" type="text" name="fbcomment[width]" value="<?php echo $fboptn['width']; ?>" />px 
								<small> - <?php _e( 'For Responsive Look use 100% as width', 'facebook-comment-by-vivacity' ); ?> 
								<?php _e( 'Default comment box width is', 'facebook-comment-by-vivacity' ); ?> <strong>500px</strong></small>
								</td>
							</tr>
							<tr>
								<th><label for="title"><?php _e( 'Title', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="title" type="text" name="fbcomment[title]" value="<?php echo $fboptn['title']; ?>" /><br> 
									<input id="titlecls" type="text" name="fbcomment[titlecls]" value="<?php echo $fboptn['titlecls']; ?>" /><small> - 
									<?php _e( 'Css class name for title', 'facebook-comment-by-vivacity' ); ?> </small>
								</td>
							</tr>
							<tr>
								<th><label for="count"><?php _e( 'Show Comment Count', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="count" name="fbcomment[count]" type="checkbox" value="on" <?php checked('on', $fboptn['count']); ?> />
								</td>
							</tr>
							<tr>
								<th><label for="countmsg"><?php _e( 'Comment text', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="countmsg" type="text" name="fbcomment[countmsg]" value="<?php echo $fboptn['countmsg']; ?>" /><br> 
									<input id="countmsgcls" type="text" name="fbcomment[countmsgcls]" value="<?php echo $fboptn['countmsgcls']; ?>" /><small> - <?php _e( 'Css class name for Comment text', 'facebook-comment-by-vivacity' ); ?> </small>
								</td>
							</tr>
							<tr>
								<th><label for="pagesid"><?php _e( 'Exclude Using Page/Post IDs', 'facebook-comment-by-vivacity' ); ?>
								<small> - <?php _e( 'For exclude FB comment box, please insert page/post ids separate them with commas, like 5, 21', 'facebook-comment-by-vivacity' ); ?> </small></label>
								</th>
								<td><input id="pagesid" type="text" name="fbcomment[pagesid]" value="<?php echo $fboptn['pagesid']; ?>" /><br> 
								</td>
							</tr>
						</table>
					</div>
					...
					<h3 id="moderation" class="title"><?php _e( 'Moderation Settings', 'facebook-comment-by-vivacity' ); ?></h3>
					<!-- ------ Moderation--------- --> 
					<div id="moderationtbl" class="togglediv">
						<table class="form-table admintbl">
							<tr>
								<th><a href="https://developers.facebook.com/tools/comments<?php if ($fboptn['appID'] != "") { echo "?id=".$fboptn['appID']."&view=queue"; } ?>" target="_blank"><?php _e( 'Moderation Area', 'facebook-comment-by-vivacity' ); ?></a>
								</th>
								<td><small><?php _e( 'If you are a moderator, you will see notifications within facebook.com. If you don\'t want to have moderator status, click on "Moderation Area" and use this link to left.', 'facebook-comment-by-vivacity' ); ?></small>
								</td>
							</tr>
							<tr>
								<th><label for="appID"><?php _e( 'Moderators', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td><input id="mods" type="text" name="fbcomment[mods]" value="<?php echo $fboptn['mods']; ?>" size="50" /><br><small><?php _e( 'All admins to the App ID can moderate comments,By default. To add moderators, enter each Facebook User ID by a comma without spaces. To find your Facebook User ID', 'facebook-comment-by-vivacity' ); ?>,<a href="https://developers.facebook.com/tools/explorer/?method=GET&path=me" target="blank"><?php _e( 'click here', 'facebook-comment-by-vivacity' ); ?></a> <?php _e( 'where you will see your own. To view someone else\'s, replace "me" with their username in the input provided', 'facebook-comment-by-vivacity' ); ?></small>
								</td>
							</tr>
						</table>
					</div>
					...
					<h3 id="" class="title"><?php _e( 'Hide/Show default wp comments', 'facebook-comment-by-vivacity' ); ?></h3>
					<!-- ------ default wp comments--------- --> 
					<div id="" class="togglediv">
						<table class="form-table admintbl">
							...
							<tr>		
								<th class="rgtth"><input type="radio"  class="mode" id="fbComments_hideWpComments" name="fbcomment[hideWpComments]" onchange="setFries()" value="on" <?php checked('on', $fboptn['hideWpComments']); ?>   />
								</th>
								<td><label for="fbComments_hideWpComments"> <?php _e('Hide WordPress Default Comments System From Website', 'facebook-comment-by-vivacity'); ?></label>
								</td>
							</tr>
							...
							<div class="clr"></div>
							<tr>
								<th class="rgtth"><input type="radio" class="mode" id="selected_types" name="fbcomment[hideWpComments]" onchange="setFries()" value="selected_types" <?php checked( 'selected_types', $fboptn['hideWpComments']); ?> /> 
								</th>
								<td><label for="selected_types"> <?php _e('Hide WordPress Default Comments System On Certain Post Types', 'facebook-comment-by-vivacity'); ?></label>
								</td>
							</tr>
						</table>
						<table class="form-table admintbl posts-pages">
							<!-- ------post--------- --> 		
							<tr>
								<th><label for="posts_hideWpComments"> <?php _e('Hide WordPress Default Comments System On Posts', 'facebook-comment-by-vivacity'); ?></label>
								</th>
								<td><input type="checkbox" class="checkmode" id="posts_hideWpComments" name="fbcomment[postshideWpComments]"  value="on" <?php checked('on', $fboptn['postshideWpComments']); ?> <?php if($fboptn['hideWpComments'] == 'on'){ ?> disabled="true"  <?php	} ?> />
								</td>
							</tr>
							<!-- ------page--------- --> 		
							<tr>
								<th><label for="fbComments_hideWpComments"> <?php _e('Hide WordPress Default Comments System On Pages', 'facebook-comment-by-vivacity'); ?></label>
								</th>
								<td><input type="checkbox" class="checkmode" id="pages_hideWpComments" name="fbcomment[pageshideWpComments]"  value="on" <?php checked('on', $fboptn['pageshideWpComments']); ?> <?php if($fboptn['hideWpComments'] == 'on'){ ?> disabled="true"  <?php	} ?> />
								</td>
							</tr>
						</table>
					</div>
						<!-- ------End default wp comments--------- --> 
					...
					<div id="langtbl" class="togglediv">
						<table class="form-table admintbl">
							<tr>
								<th><label for="Language"><?php _e( 'Select Language', 'facebook-comment-by-vivacity' ); ?></label></th>
								<td>
<?php 
								$lang=array();
...
<!-- ---End of facebook App Id settings---- -->
			...
			<div class="right">
				<center>
					...
					<div class="bottom">
						<h3 id="shortcodedesc-comments" class="title">
							<?php _e( 'Shortcode For Templates', 'facebook-comment-by-vivacity' ); ?>
						</h3>
...
<?php _e( 'You can insert', 'facebook-comment-by-vivacity' ); ?> <strong>echo do_shortcode('[vivafbcomment]');</strong> <?php _e( 'code into your templates for use this shortcode', 'facebook-comment-by-vivacity' ); ?>.
										...
										<p>
										<?php _e( 'You can also use below options to override the the settings used above.', 'facebook-comment-by-vivacity' ); ?>
										</p>
										<ul>
											<li><strong>url</strong> - <?php _e( 'leave blank for current URL', 'facebook-comment-by-vivacity' ); ?>
											</li>
											<li><strong>width</strong> -  <?php _e( 'minimum must be', 'facebook-comment-by-vivacity' ); ?> <strong>350</strong>
											</li>
											<li><strong>num</strong> - <?php _e( 'number of comments', 'facebook-comment-by-vivacity' ); ?>
											</li>
											<li><strong>count</strong> - <?php _e( 'comment count on/off', 'facebook-comment-by-vivacity' ); ?>
											</li>
											<li><strong>scheme</strong> - <?php _e( 'color scheme: light/dark', 'facebook-comment-by-vivacity' ); ?>
											</li>
											<li><strong>pluginsite</strong> - <?php _e( 'enter', 'facebook-comment-by-vivacity' ); ?> "1" <?php _e( 'to link to the plugin', 'facebook-comment-by-vivacity' ); ?>
											</li>
										</ul>
					...
					<div class="bottom">
						...
						<div id="aboutauthortbl-comments" class="togglediv">  
							...
							<ul class="">
								...
								<li><a target="_blank" href="http://www.lemonpix.com/"><span class="colortext">Web Hosting</span></a></li>
 								<h3>
